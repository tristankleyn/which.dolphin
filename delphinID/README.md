...
### 🧾 Overview
<p align="justify">
delphinID models are convolutional neural networks trained to classify delphinid species by latent characteristics in low-frequency spectra averaged across detections of whistle or click vocalizations. delphinID classification is thus not based on 2D spectrograms containing vocalizations but rather 1D spectra extracted from detections. This simplified representation was found to have several benefits as a machine learning input, including improved computational efficiency and enhanced robustness of feature robustness to imperfect detection (such as fragmenting whistle detections) and background noise. For the classifiers described here and linked in the Zenodo, detections of whistles and clicks were automatically extracted in PAMGuard and calculation of detection frames was done using the R package PAMPal [1], while the delphinID models themselves were trained in Tensorflow 2.18. Interested developers are welcome to use other methods to detect vocalizations and calculate detection frames, though we recommend at least loosely following our methods to achieve similar results. 
</p>
